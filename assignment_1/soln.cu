...
__global__ void per_row_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    // --> Complete the kernel ....
    long int i = blockIdx.x;          //0 -> m-1
    long int j = threadIdx.x;         //0 -> m-1
    for(long int k = 0; k<n; k++){
        for(long int t = 0; t<n; t++){
            C[i*m*n*n + t*m*n + k*m + j] = A[i*n+k]*B[j*n+t];
        }
    }
}
...
__global__ void per_column_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    // --> Complete the kernel ....
    long int i = threadIdx.x;
    long int j = threadIdx.y + blockIdx.x* gridDim.x;
    if(j < n){
       for(long int k = 0; k<m; k++){
            for(long int t = 0; t<m; t++){
                C[k*m*n*n + m*n*j + i*m + t] = A[n*k+i]*B[t*n+j];
            }
       }
    }
}
...
__global__ void per_element_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    // --> Complete the kernel ....;p;
    long int t_i = threadIdx.x;
    long int t_j = threadIdx.y;
    long int b_i = blockIdx.x;
    long int b_j = blockIdx.y;

    long int index_c = (b_i * gridDim.y + b_j)*( 64 * 16) + t_i * 16 + t_j;
    long int index_a, index_b;
    if(index_c<m*m*n*n){
        index_a = ((index_c / (m*n))/n)*n + (index_c % (m*n))/m;
        index_b = ((index_c % (m*n)) % m)*n + ((index_c% (m*n*n)) /(m*n));
...
int main(int argc,char **argv){
    ...
    cudaMalloc(&d_a, m*n*sizeof(long int));         // --> Allocate memory for A on device 
    cudaMalloc(&d_b, m*n*sizeof(long int));         // --> Allocate memory for B on device 
    cudaMalloc(&d_c, m*m*n*n*sizeof(long int));     // --> Allocate memory for C on device 
    ...
    cudaMemcpy(d_a, h_a, m*n*sizeof(long int), cudaMemcpyHostToDevice);      // --> Copy A from Host to Device
    cudaMemcpy(d_b, h_b, m*n*sizeof(long int), cudaMemcpyHostToDevice);      // --> Copy B from Host to Device 
    ...
    cudaMemcpy(d_c, h_c, m*m*n*n*sizeof(long int), cudaMemcpyDeviceToHost);    // --> Copy C from Device to Host 
    ...
    // --> Set the launch configuration 
    dim3 grid2(ceil(float(n)/10),1,1);
    dim3 block2(n,10,1);
